  ...
  GIVEN("A shard with an empty node and an existing node with properties") {
    ...
    WHEN("an empty relationship is added") {
      ...
      THEN( "the shard should get zero relationship ids if the wrong type is asked for" ) {
        Roaring64Map it = shard.AllRelationshipIdsMap("Wrong");
        int counter = 0;
    ...
    WHEN("an empty relationship is added") {
      ...
      THEN( "the shard should get zero relationships if the wrong type is asked for" ) {
        std::vector<triton::Relationship> it = shard.AllRelationships("Wrong");
        REQUIRE(it.empty());
      }
    }
    ...
    WHEN("a bunch of empty relationships are added") {
      ...
      THEN( "the shard should get all the relationships" ) {
        Roaring64Map it = shard.AllRelationshipIdsMap();
        int counter = 0;
        for(Roaring64MapSetBitForwardIterator i = it.begin() ; i != it.end() ; i++) {
          ++counter;
        }
    ...
    WHEN("a bunch of empty relationships are added") {
      ...
      THEN( "the shard should get all the relationships" ) {
        std::vector<triton::Relationship> it = shard.AllRelationships();
        REQUIRE(it.size() == 4);
      }
    }
    ...
    WHEN("an empty relationship is added after deleting one") {
      ...
      THEN("the shard keeps the id") {
        ...
        std::pair <uint16_t, uint64_t> rel_type_incoming_node_id = shard.RelationshipRemoveGetIncoming(triton::Shard::externalToInternal(added));
    ...
    WHEN("more relationships of a different type are added") {
      ...
      THEN("the shard keeps the ids") {
        Roaring64Map it = shard.AllRelationshipIdsMap("KNOWS");
        int counter = 0;
        for(Roaring64MapSetBitForwardIterator i = it.begin() ; i != it.end() ; i++) {
          ++counter;
        }
        ...
        it = shard.AllRelationshipIdsMap("FRIENDS");
        counter = 0;
        for(Roaring64MapSetBitForwardIterator i = it.begin() ; i != it.end() ; i++) {
          ++counter;
        }
        ...
        it = shard.AllRelationshipIdsMap(1);
        counter = 0;
        for(Roaring64MapSetBitForwardIterator i = it.begin() ; i != it.end() ; i++) {
          ++counter;
        }
        ...
        it = shard.AllRelationshipIdsMap(3);
        counter = 0;
        for(Roaring64MapSetBitForwardIterator i = it.begin() ; i != it.end() ; i++) {
          ++counter;
        }
    ...
    WHEN("more relationships of a different type are added") {
      ...
      THEN("the shard keeps them") {
        std::vector<triton::Relationship> it = shard.AllRelationships("KNOWS");
        REQUIRE(it.size() == 2);
        ...
        it2 = shard.AllRelationshipIds((uint64_t) 2,(uint64_t)3);
        REQUIRE(it2.size() == 3);
      }
    }
    ...
    WHEN("more relationships of a different type are added") {
      ...
      THEN("the shard keeps them") {
        std::vector<triton::Relationship> it = shard.AllRelationships((uint16_t)1);
        REQUIRE(it.size() == 2);

        it = shard.AllRelationships((uint16_t)3);
        REQUIRE(it.size() == 3);
      }
    }
  }
