...
SCENARIO( "Shard can handle Node Types", "[node_types]" ) {
  ...
  GIVEN("A shard with three nodes") {
    ...
    uint64_t node2id = shard.NodeAddEmpty("Node", 1, "two");
    uint64_t node3id = shard.NodeAddEmpty("Node", 1, "three");
    ...
    WHEN("add a node") {
      shard.NodeTypeInsert("Person", 2);
      shard.NodeAddEmpty("Person", 2, "four");
    ...
    WHEN("add new nodes") {
      ...
      shard.NodeAddEmpty("User", 2, "some user");
      shard.NodeAddEmpty("Thing", 3, "some thing");
      shard.NodeAddEmpty("Thing", 3, "some thing else");
