...
future<std::unique_ptr<reply>> Lua::PostLuaHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
  // If the script is missing
  if (req->content.empty()) {
    rep->write_body("json", std::move(json::stream_object("Empty script")));
