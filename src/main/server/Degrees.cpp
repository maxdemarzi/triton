...
future<std::unique_ptr<reply>> Degrees::GetDegreeHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
  ...
  if (valid_type && valid_key) {
    ...
    if(options_string.empty()) {
      // Get Node Degree
      return parent.graph.shard.local().NodeGetDegreePeered(req->param[Server::TYPE], req->param[Server::KEY])
        .then([rep = std::move(rep)] (uint64_t degree) mutable {
               rep->write_body("json", std::move(json::stream_object(degree)));
    ...
    case 1:
      ...
      return parent.graph.shard.local().NodeGetDegreePeered(req->param[Server::TYPE], req->param[Server::KEY], direction)
        .then([rep = std::move(rep)] (uint64_t degree) mutable {
               rep->write_body("json", std::move(json::stream_object(degree)));
               return make_ready_future<std::unique_ptr<reply>>(std::move(rep));
        });
    case 2: {
      ...
      if (rel_types.size() == 1) {
        return parent.graph.shard.local().NodeGetDegreePeered(req->param[Server::TYPE], req->param[Server::KEY], direction, rel_types[0])
          .then([rep = std::move(rep)] (uint64_t degree) mutable {
                 rep->write_body("json", std::move(json::stream_object(degree)));
                 return make_ready_future<std::unique_ptr<reply>>(std::move(rep));
          });
      ...
      return parent.graph.shard.local().NodeGetDegreePeered(req->param[Server::TYPE], req->param[Server::KEY], direction, rel_types)
        .then([rep = std::move(rep)] (uint64_t degree) mutable {
               rep->write_body("json", std::move(json::stream_object(degree)));
               return make_ready_future<std::unique_ptr<reply>>(std::move(rep));
        });
...
future<std::unique_ptr<reply>> Degrees::GetDegreeByIdHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
