...
future<std::unique_ptr<reply>> RelationshipProperties::GetRelationshipPropertyByIdHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
  uint64_t id = Server::validate_id(req, rep);

  if (id > 0) {
    return parent.graph.shard.local().RelationshipPropertyGetPeered(id, req->param[Server::PROPERTY])
...
future<std::unique_ptr<reply>> RelationshipProperties::PutRelationshipPropertyByIdHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
  uint64_t id = Server::validate_id(req, rep);
  bool valid_property = Server::validate_parameter(Server::PROPERTY, req, rep, "Invalid property");

  if (id > 0 && valid_property) {
    return parent.graph.shard.local().RelationshipPropertySetFromJsonPeered(id, req->param[Server::PROPERTY], req->content.c_str())
...
future<std::unique_ptr<reply>> RelationshipProperties::DeleteRelationshipPropertyByIdHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
  uint64_t id = Server::validate_id(req, rep);

  if (id > 0) {
    return parent.graph.shard.local().RelationshipPropertyDeletePeered(id, req->param.at(Server::PROPERTY))
      .then([rep = std::move(rep)] (const std::any& property) mutable {
             Server::convert_property_to_json(rep, property);
             return make_ready_future<std::unique_ptr<reply>>(std::move(rep));
      });
...
future<std::unique_ptr<reply>> RelationshipProperties::GetRelationshipPropertiesByIdHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
  uint64_t id = Server::validate_id(req, rep);
...
future<std::unique_ptr<reply>> RelationshipProperties::PostRelationshipPropertiesByIdHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
  uint64_t id = Server::validate_id(req, rep);
...
future<std::unique_ptr<reply>> RelationshipProperties::PutRelationshipPropertiesByIdHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
  uint64_t id = Server::validate_id(req, rep);
...
future<std::unique_ptr<reply>> RelationshipProperties::DeleteRelationshipPropertiesByIdHandler::handle(const sstring &path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) {
  uint64_t id = Server::validate_id(req, rep);
